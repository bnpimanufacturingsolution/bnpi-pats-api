...
- Scope amendment (2026-07-13): also touch `app/create-app.ts`, `Dockerfile`, `package.json`, `webpack.config.js`, `index.ts`, and `.gitignore` only to mount the new PATS module, ensure its already-separated generated Prisma client is produced and packaged/ignored as a build artifact, and let the PATS-only Compose profile start without an unconditional legacy Mongo connection. This is required to make the new route reachable and runnable; no existing legacy registration, auth, or workspace-membership behavior may be removed or changed.
