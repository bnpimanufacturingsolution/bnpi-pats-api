...
describe("Template Controller", () => {
	...
	describe(".getAll()", () => {
		...
		it("should group templates by type field", async function () {
			...
			expect(sentData.data.templates).to.be.an("object");
		...
		it("should handle pagination parameters", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { page: "2", limit: "5", sort: "name", order: "asc", document: "true" };
			await templateController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
		});
		...
		it("should handle field selection", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { fields: "name,type", document: "true" };
			await templateController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
		});
		...
		it("should handle documents parameter", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { documents: "true", document: "true" };
			await templateController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
		});
		...
		it("should handle count parameter", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { count: "true", document: "true" };
			await templateController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
		});
	...
	describe("Edge Cases and Integration", () => {
		it.skip("should handle empty request body (requires Zod middleware)", async function () {
			this.timeout(TEST_TIMEOUT);
			req.body = {};
			await templateController.create(req as Request, res, next);
			expect(statusCode).to.equal(400);
		...
		it.skip("should handle null request body (requires Zod middleware)", async function () {
			this.timeout(TEST_TIMEOUT);
			req.body = null;
			await templateController.create(req as Request, res, next);
			expect(statusCode).to.equal(400);
		...
		it.skip("should handle undefined request body (requires Zod middleware)", async function () {
			this.timeout(TEST_TIMEOUT);
			req.body = undefined;
			await templateController.create(req as Request, res, next);
			expect(statusCode).to.equal(400);
		...
		it.skip("should handle very long template name (requires Zod middleware)", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "A".repeat(1000), // Very long name (exceeds 255 char limit)
				description: "Template with very long name",
			};
			req.body = createData;
			await templateController.create(req as Request, res, next);
			expect(statusCode).to.equal(400); // Should fail validation (max 255 chars)
			expect(sentData).to.have.property("status", "error");
		});
		...
		it("should handle very large page numbers", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { page: "999999", limit: "10", document: "true" };
			await templateController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
		});
		...
		it("should handle very large limit values", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { page: "1", limit: "999999", document: "true" };
			await templateController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
		});
		...
		it("should handle empty string values", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { page: "", limit: "", sort: "", order: "", document: "true" };
			await templateController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(400);
			expect(sentData).to.have.property("status", "error");
		});
		...
		it("should handle whitespace-only values", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { page: "   ", limit: "   ", sort: "   ", document: "true" };
			await templateController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(400);
			expect(sentData).to.have.property("status", "error");
		});
		...
		it("should handle missing required fields in update", async function () {
			...
			req.body = {}; // Empty body
			await templateController.update(req as Request, res, next);
			expect(statusCode).to.equal(500); // 500 in test env without middleware
			expect(sentData).to.have.property("status", "error");
		});
