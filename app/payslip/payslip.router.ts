...
import { CreatePayslipSchema, UpdatePayslipSchema } from "../../zod/payslip.zod";
...
export const router = (route: Router, controller: IController): Router => {
	 ...
	 *     description: Retrieve a specific payslip by its unique identifier with optional field selection
	 *     tags: [Payslip]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *         description: Payslip ID (MongoDB ObjectId format)
	 ...
	 */
	// Cache individual payslip with predictable key for invalidation
	routes.get(
		"/:id",
		validateWorkspaceId,
		validateObjectId("id"),
		cache({
			...
			keyGenerator: (req: Request) => {
				...
				return `cache:payslip:byId:${req.params.id}:${fields}`;
			},
		}),
		controller.getById,
	);
	 ...
	 *     description: Retrieve payslips with advanced filtering, pagination, sorting, field selection, and optional grouping
	 *     tags: [Payslip]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *           type: string
	 *           enum: ["true"]
	 *         description: Include payslip documents in response
	 *       - in: query
	 *         name: pagination
	 *         required: false
	 *         schema:
	 ...
	 *         $ref: '#/components/responses/InternalServerError'
	 */
	// Cache payslip list with predictable key for invalidation
	routes.get(
		"/",
		validateWorkspaceId,
		cache({
			...
			keyGenerator: (req: Request) => {
				...
				return `cache:payslip:list:${queryKey}`;
			},
		}),
		controller.getAll,
	);
	 ...
	 *     description: Create a new payslip with the provided data
	 *     tags: [Payslip]
	 *     security:
	 *       - bearerAuth: []
	 *     requestBody:
	 ...
	 *                 type: string
	 *                 minLength: 1
	 *                 description: Payslip name
	 *                 example: "Email Welcome Payslip"
	 *               description:
	 *                 type: string
	 *                 description: Payslip description
	 *                 example: "Welcome email payslip for new users"
	 *               type:
	 *                 type: string
	 *                 enum: ["email", "sms", "push", "form"]
	 *                 description: Payslip type for categorization
	 *                 example: "email"
	 *               isDeleted:
	 *                 type: boolean
	 ...
	 *         $ref: '#/components/responses/InternalServerError'
	 */
	routes.post("/", validateWorkspaceId, transformFormData, validate(CreatePayslipSchema), controller.create);
	 ...
	 *     description: Update payslip data by ID (partial update)
	 *     tags: [Payslip]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *                 minLength: 1
	 *                 description: Payslip name
	 *                 example: "Updated Email Payslip"
	 *               description:
	 *                 type: string
	 *                 description: Payslip description
	 *                 example: "Updated description for the payslip"
	 *               type:
	 *                 type: string
	 *                 enum: ["email", "sms", "push", "form"]
	 *                 description: Payslip type for categorization
	 ...
	 *     description: Permanently delete a payslip by ID
	 *     tags: [Payslip]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
