...
import { ExpressApp, AuthenticatedRequest, RouterLayer } from "../types";
...
export const generateEndpointsFromApp = async (req: Request): Promise<Endpoint[]> => {
	...
	try {
		...
		const app = (req as AuthenticatedRequest).app as ExpressApp;

		// If app is not available, try to get it from the global scope
		if (!app || !app.router) {
			// Try to access the app from the global scope
			const appInstance = (global as unknown as GlobalWithApp).app;
...
export const generateEndpointsFromAppInstance = async (app: ExpressApp): Promise<Endpoint[]> => {
	const endpoints: Endpoint[] = [];
	...
	try {
		...
		const moduleMap = new Map<unknown, string>(); // Map router instances to their detected names
		...
		const analyzeRouters = (router: { stack?: RouterLayer[] }) => {
			...
			router.stack.forEach((layer) => {
				if (layer.name === "router" && layer.handle?.stack) {
					const subRoutes =
						layer.handle.stack
							?.filter((sublayer) => sublayer.route)
							?.map((sublayer) => sublayer.route?.path || "") || [];
					...
					if (
						...
						const handlers =
							layer.handle.stack
								?.filter((sublayer) => sublayer.route)
								?.map((sublayer) => sublayer.route?.stack?.[0]) || [];

						const handlerNames = handlers
							.map((h) => (h && typeof h === "object" && "name" in h ? (h as { name?: string }).name : undefined))
							.filter((name): name is string => !!name && name !== "anonymous");
		...
		const extractAllRoutes = (router: { stack?: RouterLayer[] }, basePath = "/api", currentModule = "API") => {
			if (!router || !router.stack) return;
				...
				if (layer.route) {
					...
					methods.forEach((method: string) => {
						...
						endpoints.push({
							...
							tags: [module],
							handler: handler && typeof handler === "object" && "name" in handler ? (handler as { name?: string }).name || "anonymous" : "anonymous",
						});
					});
...
const extractRoutesFromApp = (app: ExpressApp, endpoints: Endpoint[]) => {
	console.log("Extracting routes from app, router stack length:", app.router?.stack?.length || 0);

	const extractRoutes = (router: { stack?: RouterLayer[] }, basePath = "") => {
		...
		console.log(
			`Extracting routes from router with ${router.stack?.length || 0} layers, basePath: ${basePath}`,
		);
		router.stack?.forEach((layer, index: number) => {
			if (layer.route) {
	...
	if (app.router && app.router.stack) {
		app.router.stack.forEach((layer, index: number) => {
			if (layer.name === "router" && layer.handle?.stack) {
				const routePaths = layer.handle.stack
					.filter((sublayer) => sublayer.route)
					.map((sublayer) => sublayer.route?.path || "");
...
const extractSummaryFromHandler = (handler: unknown): string => {
...
const extractTagsFromHandler = (handler: unknown): string[] => {
	if (handler && typeof handler === "function") {
		const handlerStr = handler.toString();
