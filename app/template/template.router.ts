...
export const router = (route: Router, controller: IController): Router => {
	 ...
	 *     description: Retrieve a specific template by its unique identifier with optional field selection
	 *     tags: [Template]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 */
	// Cache individual template with predictable key for invalidation
	routes.get(
		"/:id",
		validateWorkspaceId,
		validateObjectId("id"),
		cache({
			...
			keyGenerator: (req: Request) => {
				...
				return `cache:template:byId:${req.params.id}:${fields}`;
			},
		}),
		controller.getById,
	);
	 ...
	 *     description: Retrieve templates with advanced filtering, pagination, sorting, field selection, and optional grouping
	 *     tags: [Template]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *         $ref: '#/components/responses/InternalServerError'
	 */
	// Cache template list with predictable key for invalidation
	routes.get(
		"/",
		validateWorkspaceId,
		cache({
			...
			keyGenerator: (req: Request) => {
				...
				return `cache:template:list:${queryKey}`;
			},
		}),
		controller.getAll,
	);
	 ...
	 *         $ref: '#/components/responses/InternalServerError'
	 */
	routes.post("/", validateWorkspaceId, transformFormData, validate(CreateTemplateSchema), controller.create);
