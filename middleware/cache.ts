...
export class CacheManager {
	...
	/**
	 * Invalidate cache by pattern using SCAN (non-blocking, unlike KEYS)
	 */
	async invalidatePattern(pattern: string): Promise<number> {
		try {
			...
			// Use SCAN instead of KEYS to avoid blocking Redis (ioredis syntax)
			do {
			...
			if (totalDeleted > 0) {
				logger.info(`Invalidated ${totalDeleted} cache entries matching pattern: ${pattern}`);
			}
			return totalDeleted;
		} catch (error) {
